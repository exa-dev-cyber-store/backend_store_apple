...
export const authenticAppleProducts = [
  // ==================== iPHONE ====================
  {
    categoryName: 'iPhone',
...
export async function seedAppleProducts(): Promise<void> {
  ...
  for (const name of categoryNames) {
    ...
    if (!cat) {
      cat = await CategoryModel.create({ name });
  ...
  const docs = authenticAppleProducts.map((item) => ({
    name: item.name,
    price: item.price,
