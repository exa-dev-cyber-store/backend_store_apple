...
class EmailService {
            ...
            if (apiKey && apiKey.trim() !== '') {
                try {
                    ...
                    if (!response.ok) {
                        ...
                        return { success: false, note: (data === null || data === void 0 ? void 0 : data.message) || 'Resend delivery rejected' };
                    }
                    console.log(`[EmailService] Email sent successfully via Resend to ${to}. Email ID:`, data === null || data === void 0 ? void 0 : data.id);
     ...
     */
    static sendPasswordResetEmail(_a) {
        return __awaiter(this, arguments, void 0, function* ({ to, name, resetUrl, token }) {
            const isAppleRelay = to.toLowerCase().endsWith('@privaterelay.appleid.com');
    ...
    static sendPaymentReceiptEmail(_a) {
        return __awaiter(this, arguments, void 0, function* ({ order, user, invoice }) {
            var _b;
            ...
            const recipientName = user.name || ((_b = order.delivery_address) === null || _b === void 0 ? void 0 : _b.name) || 'Valued Customer';
            ...
            const rawPm = (order.payment_method || (invoice === null || invoice === void 0 ? void 0 : invoice.payment_method) || '').toLowerCase();
            if (rawPm.includes('qris'))
                paymentMethodDisplay = 'QRIS (GoPay / OVO / ShopeePay / BCA)';
            else if (rawPm.includes('bca'))
                paymentMethodDisplay = 'BCA Virtual Account';
            else if (rawPm.includes('bni'))
                paymentMethodDisplay = 'BNI Virtual Account';
            else if (rawPm.includes('bri'))
                paymentMethodDisplay = 'BRI Virtual Account';
            else if (rawPm.includes('mandiri') || rawPm.includes('echannel'))
                paymentMethodDisplay = 'Mandiri Bill Payment';
            else if (rawPm.includes('permata'))
                paymentMethodDisplay = 'Permata Virtual Account';
            else if (rawPm.includes('gopay'))
                paymentMethodDisplay = 'GoPay Direct Charge';
            else if (rawPm.includes('cstore') || rawPm.includes('indomaret'))
                paymentMethodDisplay = 'Retail Store (Indomaret / Alfamart)';
            else if (rawPm)
                paymentMethodDisplay = rawPm.toUpperCase();
            const deliveryAddr = order.delivery_address || {};
            const itemsHtml = items.map((item, idx) => {
                var _a;
                const lineTotal = (item.price || 0) * (item.quantity || 1);
                const sku = String(((_a = item._id) === null || _a === void 0 ? void 0 : _a._id) || item._id || '').slice(-8).toUpperCase();
                return `
...
EmailService.defaultFrom = process.env.RESEND_FROM_EMAIL || 'Cyber Store <noreply@resend.dev>';
